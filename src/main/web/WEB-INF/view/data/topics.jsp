        ...
        <div class="container-fluid wam-not-padding-xs">
            ...
            <div class="panel panel-default wam-margin-panel">
                <div class="panel-heading wam-page-title">
                    <h3 class="wam-margin-bottom-0 wam-margin-top-0">Таблица постов</h3>
                </div>
                <div class="panel-body wam-not-padding">
                    <table class="table table-striped table-bordered table-text wam-margin-top-2 dataTable no-footer ">
                        <thead>
                        <tr>
                            <td onclick="location.href='?sort=title,${sortDirection}';" class="wam-cursor">Тема</td>
                            <td onclick="location.href='?sort=rating,${sortDirection}';" class="wam-cursor">Рейтинг</td>
                            <td onclick="location.href='?sort=author,${sortDirection}';" class="wam-cursor">Автор</td>
                            <td onclick="location.href='?sort=countComments,${sortDirection}';" class="wam-cursor">Комментарии</td>
                        </tr>
                        </thead>
                        <tbody>
                        ...
                        <c:forEach items="${topics}" var="topic">
                            <tr class="">
                                <td><a href="${topic.getUrl()}">${topic.getTitle()}</a></td>
                                <td>${topic.getRating()}</td>
                                <td>${topic.getAuthor()}</td>
                                <td>${topic.getCountComments()}</td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
